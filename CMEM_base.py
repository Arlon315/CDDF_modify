    ...
    def __init__(
        ...
        if selective_scan_fn is None:
            raise ImportError(
                ...
                "in the training environment before using CMEM_base."
            )
...
class CMEMBaseFusion(nn.Module):
    dual_input = True

    def __init__(self, dim=64, share_mamba=False, use_checkpoint=True):
        super(CMEMBaseFusion, self).__init__()
    ...
    def forward(self, ir_base_feature, vi_base_feature):
        cross = self.cross_mixer(
            self.ir_norm(ir_base_feature),
            self.vi_norm(vi_base_feature),
        )
        cross = self.merge(cross)
        return ir_base_feature + vi_base_feature + cross
